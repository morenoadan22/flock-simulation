	...
	void Update () {
        ...
        for( int i = 0; i < objects.Length; i++)
        {
            ...
            if( transform != transformComponent)
            {
                ...
                //Add push value if the magnitude, the length of the vector,
                //is less than followRadius to the leader
                if( d < followRadius)
                {
                    //calculate the velocity, the speed of the object, based on
                    //the avoidance distance between flocks if the current magnitude
                    //is less than the specified avoidance radius
